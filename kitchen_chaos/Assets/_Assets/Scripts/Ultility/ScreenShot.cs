...
public class ScreenShot : EditorWindow
{
    ...
    private static void TakeScreenshot()
    {
        ...
        // In order to avoid loading Texture2D into memory destroy it
        ...
        var path = Path.Combine(Application.dataPath, "Screenshots", "Screenshot" + stampString + ".png");
        File.WriteAllBytes(path, bytes);

        // Refresh the AssetsDatabase so the file actually appears in Unity
        AssetDatabase.Refresh();

        Debug.Log("New Screenshot taken at: " + path);
    }
}
